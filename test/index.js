...
test("render-only component (non-contextual)", async (t) => {
  function ComponentVeryFun(html, props) {
    return html` <div data-id="${props.id}"></div> `
  }
...
test("attach to dom with shadow", async (t) => {
  Tonic.add(
    class ShadowComponent extends Tonic {
      constructor(o) {
        super(o)
...
test("pass props", async (t) => {
  Tonic.add(
    class ComponentBB extends Tonic {
      render() {
...
test("sync lifecycle events", async (t) => {
  ...
  class XQuxx extends Tonic {
    ...
    connected() {
      ...
      const expectedRE =
        /<x-quxx><div class="quxx"><x-bazz><div class="bar"><\/div><\/x-bazz><\/div><\/x-quxx>/
...
test("check that composed elements use (and re-use) their initial innerHTML correctly", async (t) => {
  ...
  t.equal(
    kTextAfterSetProps,
    "1",
    "The text of the inner-most child was rendered correctly"
  )
})
...
test("default props", async (t) => {
  ...
  const expectedRE =
    /<instance-props str="0x"><div>{"num":100,"str":"0x"}<\/div><\/instance-props>/
